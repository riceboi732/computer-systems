...
int conditional(int x, int y, int z) {
  //if x is 0, return z
  //if x is anything else, return y
  int notx = !x;
  return (y & (notx + ~0)) | (z & ((notx << 31)>>31));
...
int logicalNeg(int x) {
  //if x and -x are the same, return 1. Else return 0
  int notx1 = ~x + 1;
  return (1 + (notx1 ^ x)) & (~(notx1 ^ x) >> 31) & ((x >> 31) + 1);
}
/*
...
int isLessOrEqual(int x, int y) {
   ...
   int case1 = (!!((x + ~y ) >> 31));
   int case2 = (!(x ^ (1 << 31)));
   int case3 = ((defnotx & !defnoty));
   int case4 = (!(x ^ y));
   int case5 = (defnoty & !defnotx);
   // printf("%d\n",case1);
   // printf("%d\n",case2);
   // printf("%d\n",case3);
   // printf("%d\n",case4);
  return (((case1 & ~case3) | case2) | case4) | case5;
}
/*
 ...
 *   Max ops: 30
 *   Rating: 5
 *   Advice: use shifts and masking to break uf into sign, exponent, and fraction.
 *           Handle cases: denormalized, NaN or INF, too large for int
 *           Apply the formula to return the appropriate value
 */
int floatFloat2Int(unsigned uf) 
{
   int expbit = (uf >> 23) & 0xFF;
   int fracbit = uf & 0x7FFFFF;
   int e = expbit - 127;
   ...
   if(e > 30){
    ...
    fracbit = fracbit | 0x800000;

    if (e >= 23){
      fracbit = fracbit << (e-23);
    }
    else{
      fracbit = fracbit >> (23 -e);
    }
